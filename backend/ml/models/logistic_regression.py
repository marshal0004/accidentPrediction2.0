...
def train_logistic_regression(X_train, y_train, X_test, y_test, feature_names, label_mapping, save_dir):
    print(f"\n  {'='*50}")
    ...
    try:
        ...
        sorted_idx = np.argsort(coefs)[-top_n:][::-1]
        for idx in sorted_idx:
            fname = feature_names[idx] if idx < len(feature_names) else f"feature_{idx}"
            feature_importances.append({"name": fname, "importance": round(float(coefs[idx]), 6)})

        print(f"\n  Top 10 Feature Importances (|coefficients|):")
        for fi in feature_importances[:10]:
            print(f"    {fi['name']}: {fi['importance']:.6f}")
    except Exception as e:
        print(f"  [WARNING] Could not extract LR coefficients: {e}")

    model_path = os.path.join(save_dir, "LogisticRegression_model.joblib")
