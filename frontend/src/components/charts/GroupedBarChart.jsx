  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 40 }}>
        ...
        <XAxis dataKey="name" tick={{ fill: '#8899A6', fontSize: 9 }} angle={-30} textAnchor="end" tickLine={false} />
        ...
        {data.datasets.map((ds, i) => (
          <Bar key={ds.label} dataKey={ds.label} fill={ds.color || CHART_COLORS[i]} radius={[4, 4, 0, 0]} animationDuration={1200} />
        ))}
      </BarChart>
    </ResponsiveContainer>
  );
